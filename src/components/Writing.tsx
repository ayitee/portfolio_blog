export default function Writing() {
  ...
  <div className="sticky top-0 z-20 -mx-6 mb-4 w-screen backdrop-blur px-6 py-5 md:-mx-12 md:px-12 lg:sr-only lg:relative lg:top-auto lg:mx-auto lg:w-full lg:px-0 lg:py-0 lg:opacity-0"
    ...
    <h2 className="text-sm font-bold uppercase tracking-widest lg:sr-only" style={{color:'var(--color-accent)'}}>Writing</h2>
      </div>
            ...
            <li key={index} className="mb-12">
              <div className="group relative grid gap-4 pb-1 transition-all sm:grid-cols-8 sm:gap-8 md:gap-4 lg:hover:!opacity-100 lg:group-hover/list:opacity-50">
                ...
                <div className="z-10 sm:order-2 sm:col-span-6">
                  ...
                  <p className="mt-2 text-sm leading-normal text-[var(--color-text)] opacity-70">{article.description}</p>
                  
                  <p className="mt-2 text-xs font-semibold uppercase tracking-wide text-[var(--color-text)] opacity-60">
                    {article.year}
                  </p>
                </div>
                  ...
                  <div className="relative">
                    <div className="border-2 rounded overflow-hidden transition-colors duration-300"
                      ...
                      <div className="aspect-video flex items-center justify-center" style={{background:'color-mix(in srgb, var(--color-surface) 40%, transparent)'}}>
                        <div className="text-[var(--color-text)] opacity-60 text-sm">Article Image</div>
                      </div>
                    </div>
                  </div>
                </div>
